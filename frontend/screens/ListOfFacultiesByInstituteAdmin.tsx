...
type ListOfFacultiesByInstituteAdminProps = NativeStackScreenProps<
  RootStackParamList,
  'ListOfFacultiesByInstituteAdmin'
>;

const ListOfFacultiesByInstituteAdmin = ({
  route,
}: ListOfFacultiesByInstituteAdminProps) => {
  const navigation =
  ...
  const retrieveData = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://10.200.6.190:8080/getAllFacultyByInstituteAdmin?email=${emailId}`,
      );
    ...
    <View style={styles.container}>
      <View style={styles.navigationBar}>
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          ...
          />
          <Text style={styles.navigationText}>Manage Faculty</Text>
        </View>
      ...
      <ScrollView style={{width: '100%'}}>
        <View style={styles.subContainer}>
          ...
          <View style={styles.courseContainer}>
            <Text style={styles.courseContainerTitle}>Faculties</Text>
            {loading ? (
...
export default ListOfFacultiesByInstituteAdmin;
